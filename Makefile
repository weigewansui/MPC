...
SOURCES     = src/asctecCommIntf.c src/util.c src/filter_util.c main.c logger/logger.c navigation/read_imu.c \
	navigation/nav.c navigation/hummingBirdState.c logger/imu_logger.c \
	logger/kalman_logger.c logger/monte_carlo_logger.c LLP_control/main.c logger/accel_logger.c
CPP_SOURCES = control/cmmd_gen.cpp
EXECUTABLE  = bin/aci_recvar bin/logger bin/transmit_log bin/control_test bin/logger bin/accel_logger
...
$(EXECUTABLE): $(LCM_O) $(OBJECTS) 
	...
	$(CC) $(LFLAGS)-o bin/accel_logger $(LCM_O) logger/accel_logger.o $(ASCLIB) $(UTILS) $(LIBS) $(LDFLAGS)
	$(CC) $(LFLAGS)-o bin/monte_carlo_logger $(LCM_O) logger/monte_carlo_logger.o $(ASCLIB) $(UTILS) $(LIBS) $(LDFLAGS)
	$(CC) $(LFLAGS)-o bin/kalman_logger $(LCM_O) logger/kalman_logger.o $(ASCLIB) $(UTILS) $(LIBS) $(LDFLAGS)
